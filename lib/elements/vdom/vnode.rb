    ...
    class VNode
      ...
      def class(*args)
        if args.size == 0
          ...
          if classname != nil
            classname.split(/\s/).each do |c|
              @classes.add(classname) unless c.strip.empty?
            end
          end
      ...
      def children(children_vnodes = nil)
        if children_vnodes
          children_vnodes.each { |vnode| child(vnode) }
          self
        else
          @children
        end
      ...
      def insert_before(sibling_element)
        insert_queue = []
        new_element = to_element(insert_queue)
        ...
        if dom_children.size > 0 && children.size > 0
          ...
          while dom_list.left_idx <= dom_list.right_idx && vnode_list.left_idx <= vnode_list.right_idx do
            if dom_list.left.nil?
              ...
              # Else create the new element and insert it
                ...
                element = vnode_list.left.to_element()
                vnode_list[vnode_list.left_idx] = element
          ...
          if dom_list.left_idx > dom_list.right_idx
            ...
            (vnode_list.left_idx..vnode_list.right_idx).each do |idx|
              ...
              new_child = vnode.to_element(insert_queue)
          ...
          children.each { |child_vnode| patch_insert(parent_element, child_vnode.to_element(insert_queue)) }
      ...
      def to_element(insert_queue = [])
        create_dom_node.tap do |element|
          children.each { |vnode| api.append_child(element, vnode.to_element(insert_queue)) }
      ...
      def inspect
        "<VNode tag=\"#{tag}\" key=\"#{key}\" children=#{children.size}>"
      end

      protected
      def create_dom_node
        api.create_element(tag)
      end
