...
namespace :test do
  ...
  task :auto do
    ...
    listener = Listen.to('.', 'lib', 'test', ignore: ignore) do |modified, added, removed|
      added.each { |path| log_warning "#{relative(path)} was added." }
      modified.each { |path| log_warning "#{relative(path)} was modified." }
      removed.each { |path| log_warning "#{relative(path)} was removed." }
